...
def single_linkage(points, first, second):
	'''
	TO DO
	...
	for x in first.items:
		for y in second.items:
			d = euclidean_distance(points[x], points[y])
			if d < min_prox:
				min_prox = d
	return min_prox


def complete_linkage(points, first, second):
	'''
	TO DO
	...
	'''
	max_prox = 0
	for x in first._items:
		for y in second._items:
			d = euclidean_distance(points[x], points[y])
			if d > max_prox:
				max_prox = d
	return max_prox
...
class MyAgglomerativeClustering():
	...
	def find_clusters_to_merge(self):
		...
		for i in range(len(self._clusters) - 1):
			if self._clusters[i].used:
				for j in range(i + 1, len(self._clusters)):
					if self._clusters[j].used:
						if self._proximity_matrix[i, j] > 0 and self._proximity_matrix[i, j] < min_prox:
							first = i
							second = j
							min_prox = self._proximity_matrix[i, j]
		# print(first, second, min_prox)
		return first, second


	def merge_cluster(self, first, second):
		'''
		TO DO
		...
		'''
		new_cluster = MyCluster()
		new_cluster.extend(self._clusters[first].items)
		new_cluster.extend(self._clusters[second].items)
		# print("[%d %d] -> %d:" % (first, second, len(self._clusters)), new_cluster.items)
		
		self._clusters[first].release()
		self._clusters[second].release()
	...
	def update_proximity(self, new_cluster):
		...
		for i in range(len(self._clusters)):
			if self._clusters[i].used and i != new_cluster:
				self._proximity_matrix[i, new_cluster] = self._linkage_func(self._items, self._clusters[i], self._clusters[new_cluster])
				# print(i, new_cluster, self._proximity_matrix[i, new_cluster])
...
def compare_results(history, filename):
...
if __name__=='__main__':
	for i in range(4):
		...
		# Compare the results with sklearn.cluster.AgglomerativeClustering
		single=AgglomerativeClustering(n_clusters=1,affinity='euclidean',linkage='single')
		model_single = single.fit(testdata)
		np.savetxt(file_single_answer, model_single.children_, fmt='%d', delimiter=',')

		complete=AgglomerativeClustering(n_clusters=1,affinity='euclidean',linkage='complete')
		model_complete = complete.fit(testdata)
		np.savetxt(file_complete_answer, model_complete.children_, fmt='%d', delimiter=',')

		print(compare_results(history_single, file_single_answer), compare_results(history_complete, file_complete_answer))
